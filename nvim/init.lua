...
-- [[ Treesitter Compatibility Shim ]]
-- Fix for telescope.nvim with newer Neovim/treesitter versions
-- Store treesitter configuration from setup
local ts_config_store = {}

-- Create the compatibility module for nvim-treesitter.configs
-- This provides the old API that Telescope expects
    ...
    is_enabled = function(mod, lang, bufnr)
      ...
      if mod_config.disable then
        if type(mod_config.disable) == 'function' then
          return not mod_config.disable(lang, bufnr)
        elseif type(mod_config.disable) == 'table' then
...
-- Also add compatibility for parsers.ft_to_lang
